                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache)
{
...

    {
		...
		// tq: bitcore Add memory address index
        if (fAddressIndex) {
            pool.addAddressIndex(entry, view);
        }

        // tq: bitcore Add memory spent index
        if (fSpentIndex) {
...
DisconnectResult CChainState::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view)
{
            ...
            for (unsigned int j = tx.vin.size(); j-- > 0;) {
                ...
                if (fAddressIndex) {
                    ...
                    if (prevout.scriptPubKey.IsPayToScriptHash()) {
                        ...
                        addressUnspentIndex.push_back(std::make_pair(CAddressUnspentKey(2, uint160(hashBytes), input.prevout.hash, input.prevout.n),
                        ...
                        // restore unspent index
                        addressUnspentIndex.push_back(std::make_pair(CAddressUnspentKey(1, uint160(hashBytes), input.prevout.hash, input.prevout.n),
																CAddressUnspentValue(prevout.nValue, prevout.scriptPubKey, undo.nHeight)));
                    } else {
                        continue;
                    }
	...
	if (fAddressIndex) {
        if (!pblocktree->EraseAddressIndex(addressIndex)) {
			...
			if (!AbortNode("Failed to delete address index"))
				return DISCONNECT_FAILED;
        }
        if (!pblocktree->UpdateAddressUnspentIndex(addressUnspentIndex)) {
			//return AbortNode(state, "Failed to write address unspent index");
            if (!AbortNode("Failed to write address unspent index"))
				return DISCONNECT_FAILED;
        }
    }
