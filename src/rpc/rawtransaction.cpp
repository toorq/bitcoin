                      ...
                      int nHeight = 0, int nConfirmations = 0, int nBlockTime = 0)
{
    ...
    for (unsigned int i = 0; i < tx.vin.size(); i++) {
		...
		*/ 
        ...
        vin.push_back(in);
    } // End of for (unsigned int i = 0; i < tx.vin.size(); i++) {
...
UniValue getrawtransaction(const JSONRPCRequest& request)
{
	...
	{
		LOCK(cs_main);
		if (!GetTransaction(hash, ptx, Params().GetConsensus(), hash_block, true, blockindex)) {
			std::string errmsg;
			if (blockindex) {
    ...
    if (!fVerbose) {
        return EncodeHexTx(*ptx, RPCSerializationFlags());
    }
	...
	TxToJSONExpanded(*ptx, hash_block, result, nHeight, nConfirmations, nBlockTime);
